...
class BaseSKLearnEvaluator(BaseEvaluator):
    ...
    def evaluate(self, x, y, split):
        ...
        if self.classifier is None:
            print('fit new classifier...')
            classifier = GridSearchCV(self.evaluator, self.params, cv=ps, scoring='accuracy', verbose=0)
            self.classifier = classifier
            self.classifier.fit(x_train, y_train)
        else:
            print('reuse classifier...')
        predictions = self.classifier.predict(x_test)
        test_macro = f1_score(y_test, predictions, average='macro')
        test_micro = f1_score(y_test, predictions, average='micro')
        #test_acc = accuracy_score(y_test, predictions)
        test_roc_auc_score = roc_auc_score(y_test, predictions)
        
        return {
            'micro_f1': test_micro,
            'macro_f1': test_macro,
            'auc': test_roc_auc_score,
